...
class GreedyGardener(Gardener):
    ...
    def cultivate_garden(self) -> None:
        ...
        while self.remaining_varieties:
            ...
            if self.config['debug']['verbose']:
                constraint = " [2-species constraint]" if len(self.garden.plants) >= 2 else ""
                print(f"Iter {iteration}: {len(self.garden.plants)} placed, {len(self.remaining_varieties)} remain{constraint}")
    ...
    def print_final_analysis(self) -> None:
        ...
        species_stats = defaultdict(lambda: {'count': 0, 'total_growth': 0.0, 'sizes': [], 'interactions': []})
        ...
        for i, plant in enumerate(self.garden.plants, 1):
            ...
            interact_str = ', '.join(f"{count}{s}" for s, count in sorted(interaction_species.items()))
            
            print(f"{i:2}. {species_letter} pos=({plant.position.x:2.0f},{plant.position.y:2.0f}) "
    ...
    def _generate_candidates(self) -> List[Position]:
        ...
        if len(self.garden.plants) == 0:
            ...
            # 2nd plant: use standard geometric generation
            representative_variety = self.remaining_varieties[0]
            candidates = generate_geometric_candidates(
                self.garden,
                representative_variety,
                self.config['geometry']['angle_samples'],
                self.config['geometry']['max_anchor_pairs']
        ...
        candidates = filter_candidates(
            candidates,
            self.garden,
            self.config['geometry']['tolerance']
        )
    ...
    def _generate_multi_species_candidates(self, variety: PlantVariety) -> List[Position]:
        ...
        if len(different_species) < 2:
            ...
            if self.config['debug']['verbose']:
                print(f"  Cannot generate multi-species candidates: only {len(different_species)} different species in garden")
            return []
                    ...
                    for p2 in plants2:
                        ...
                        for factor in [0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]:
                            intersections = circle_circle_intersection(
                                p1.position, r1_interaction * factor,
                                p2.position, r2_interaction * factor
                            )
    ...
    def _prioritize_varieties(self) -> List[PlantVariety]:
        ...
        """
        if len(self.garden.plants) == 0:
            # First plant: no priority yet
            return self.remaining_varieties.copy()
        ...
        def priority_score(variety: PlantVariety) -> float:
            ...
            can_interact = any(
                p.variety.species != variety.species 
    ...
    def _find_best_placement(self, candidates: List[Position]) -> tuple:
        ...
        if self.config['debug']['verbose']:
            ...
            print(f"  Eval: {len(candidates)} pos × {len(representative_varieties)} species = {total_evaluations} combos", end="")
        ...
        for position in candidates:
            for idx, variety in enumerate(representative_varieties):
                ...
                # Only add priority bonus if there's actual imbalance
                if len(self.garden.plants) > 0:
