...
class Gardener8(Gardener):
    ...
    def cultivate_garden(self) -> None:
        ...
        spacing = 2 * max(R.radius, G.radius, B.radius) # horizontal space btwn plants along base of triangle
        ...
        dy = spacing * math.sin(math.pi / 3) # height of the triangle - this becomes vertical offset to next row
        ...
        while y < garden_h: # loop vertically through rows
            x = 0.0 if not row_shift else dx # does the horizontal shift for the row depending on the flag
            while x < garden_w: # loop horizontally through the row
                triad_plants = []
                for sp in [Species.RHODODENDRON, Species.GERANIUM, Species.BEGONIA]: # form the triad!
                    if remaining[sp]:
                        triad_plants.append(remaining[sp].pop(0))
                    else:
                        triad_plants.append(None) # but if a species is missing, just add None to the triad

                if any(triad_plants): # if the triad has a plant, place it
                    self.place_triad(*triad_plants, x, y) # at the right horizontal / vertical offset
        ...
        # Since we probably won't have perfect triads, place any leftover plants
        leftovers = [plant for sp_list in remaining.values() for plant in sp_list]
        self.place_leftovers(leftovers)
