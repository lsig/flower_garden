...
class Gardener3(Gardener):
    ...
    def cultivate_garden(self, prevent_interactions: bool = True) -> None:
        ...
        # Otherwise use clustering based approach
            ...
            if top_clusters:
                self.tile_cluster_across_garden_with_prefiltering(top_clusters, prevent_interactions=self.PREVENT_INTERACTIONS)
    
    def _check_hexagonal_condition(self) -> bool:
        exactly_three_varieties = len(self.variety_counts) == 3 
        equal_counts = all(count == list(self.variety_counts.values())[0] for count in self.variety_counts.values())
        equal_radii = all(variety.radius == list(self.species_varieties.values())[0][0].radius for variety in self.varieties)
    ...
    def calculate_cluster_score(self, plant_varieties: List[PlantVariety], coordinates: List[Position]) -> float:
        ...
        # Calculate the Area
        x_min = min(coords.x - variety.radius for variety, coords in zip(plant_varieties, coordinates))
        x_max = max(coords.x + variety.radius for variety, coords in zip(plant_varieties, coordinates))
        y_min = min(coords.y - variety.radius for variety, coords in zip(plant_varieties, coordinates))
        y_max = max(coords.y + variety.radius for variety, coords in zip(plant_varieties, coordinates))
    ...
    def find_best_triad_permutations(self, species_varieties: Dict[Species, List[PlantVariety]]) -> List[ClusterData]:
        ...
        for species_triad in species_combinations:
            ...
            variety_combinations = list(variety_product(species1_varieties, species2_varieties, species3_varieties))
        ...
        if top_triads:
            print(f"Best triad: {[f'{v.name}: {v.species.name}' for v in top_triads[0][0]]} | Score: {top_triads[0][2]:.4f}")
        
        return top_triads

    def find_best_diamond_permutations(self, species_varieties: Dict[Species, List[PlantVariety]], ranked_triads: List[ClusterData]) -> List[ClusterData]:
        ...
        if top_diamonds:
            print(f"Best diamond: {[f'{v.name}: {v.species.name}' for v in top_diamonds[0][0]]} | Score: {top_diamonds[0][2]:.4f}")
    ...
    def find_diamond_coordinates(self, diamond: List[PlantVariety], offset: Position = Position(0, 0)) -> List[Tuple[PlantVariety, Position]]:
        if len(diamond) != 4:
    ...
    def find_triad_coordinates(self, triad: List[PlantVariety], offset: Position = Position(0, 0)) -> List[Tuple[PlantVariety, Position]]:
        if len(triad) != 3:
        ...
        d12 = self.calculate_optimal_distance(variety1.radius, variety2.radius)  # Distance between variety1 and variety2
        d13 = self.calculate_optimal_distance(variety1.radius, variety3.radius)  # Distance between variety1 and variety3
        d23 = self.calculate_optimal_distance(variety2.radius, variety3.radius)  # Distance between variety2 and variety3
    ...
    def tile_cluster_across_garden_with_prefiltering(self, ranked_clusters: List[ClusterData], 
                                                   prevent_interactions: Optional[bool] = None) -> None:
        ...
        self.process_cluster_placement(radius_groups, anchor_points, placed_plants, 
                                             used_varieties, total_varieties, prevent_interactions)
    ...
    def group_clusters_by_radius_signature(self, ranked_clusters: List[ClusterData]) -> Dict[Tuple[float, ...], List[ClusterData]]:
        radius_groups = {}
    ...
    def process_cluster_placement(self, radius_groups: Dict[Tuple[float, ...], List[ClusterData]], 
                                        anchor_points: List[Tuple[float, float]], 
                                        placed_plants: List[Tuple[float, float, float, PlantVariety]], 
                                        used_varieties: Set[int], 
                                        total_varieties: int, 
                                        prevent_interactions: bool) -> None:
        placement_round = 0
        ...
        while radius_groups and anchor_points:
            ...
            best_cluster, best_radius_signature = self.find_best_available_cluster(radius_groups, used_varieties)
            ...
            print(f"Trying best available cluster. Radius pattern: {best_radius_signature} | Score: {score:.4f}")
            
            # Try to place this cluster
            placement_result = self.try_place_cluster(varieties, coordinates, score, anchor_points, placed_plants, 
                                                     used_varieties, total_varieties, prevent_interactions)
            if placement_result:
                ...
                print(f"Radius pattern {best_radius_signature} has no valid placements - removing entire pattern")
    ...
    def find_best_available_cluster(self, radius_groups: Dict[Tuple[float, ...], List[ClusterData]], 
                                   used_varieties: Set[int]) -> Tuple[Optional[ClusterData], Optional[Tuple[float, ...]]]:
    ...
    def try_place_cluster(self, varieties: List[PlantVariety], 
                          ...
                          used_varieties: Set[int], 
                          total_varieties: int, 
                          prevent_interactions: bool) -> bool:
        ...
        for anchor_x, anchor_y in anchor_points:
            ...
            for rotation_angle in range(0, 360, self.ROTATION_DEGREES):
                ...
                if self.can_place_cluster_at_anchor(rotated_coordinates, anchor_x, anchor_y, placed_plants, prevent_interactions):
                    # Calculate total area if this placement is added
                    total_area = self.calculate_total_garden_area(placed_plants, rotated_coordinates, anchor_x, anchor_y)
                    
                    possible_placements.append((rotated_coordinates, anchor_x, anchor_y, len(rotated_coordinates), total_area))
        ...
        if possible_placements:
            ...
            print(f"Placed cluster (score: {score:.4f}) at ({x:.1f}, {y:.1f}) - {plants} plants, area: {area:.2f}")
            return (x, y)
    ...
    def remove_anchor_points(self, anchor_points: List[Tuple[float, float]], 
                                                placed_plants: List[Tuple[float, float, float, PlantVariety]]) -> List[Tuple[float, float]]:
        remaining_anchors = []
    ...
    def can_place_cluster_at_anchor(self, cluster_coordinates: List[Tuple[PlantVariety, Position]], 
                                   anchor_x: float, anchor_y: float, placed_plants: List[Tuple[float, float, float, PlantVariety]], 
                                   prevent_interactions: bool = True) -> bool:
        ...
        if cluster_min_x < 0 or cluster_max_x > self.garden.width or cluster_min_y < 0 or cluster_max_y > self.garden.height:
            return False
    ...
    def calculate_total_garden_area(self, placed_plants: List[Tuple[float, float, float, PlantVariety]], new_cluster_coordinates: List[Tuple[PlantVariety, Position]], 
                                   anchor_x: float, anchor_y: float) -> float:
    ...
    def calculate_union_area(self, circles: List[Tuple[float, float, float]], 
    ...
    def place_cluster_at_anchor(self, cluster_coordinates: List[Tuple[PlantVariety, Position]], 
                               anchor_x: float, anchor_y: float, placed_plants: List[Tuple[float, float, float, PlantVariety]]) -> None:
    ...
    def place_plants_in_garden(self, placed_plants: List[Tuple[float, float, float, PlantVariety]]) -> None:
    ...
    def rotate_cluster(self, cluster_coordinates: List[Tuple[PlantVariety, Position]], angle_degrees: float) -> List[Tuple[PlantVariety, Position]]:
